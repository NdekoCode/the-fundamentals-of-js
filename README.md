...
Pour pouvoir communiquer avec un service Web il est primordiale de savoir créer des `requetes HTTP` c'est là qu'AJAX entre en jeux.
AJAX **c'est un ensemble d'objet et des fonctions** mis en place par le language Javascript à fin d'executer des requetes HTTP.
A fin des recuperer des données, nous allons créer une requete avec la methode GET, c'est la methode qui permet de demander des données, une fois la methode la methode créer et envoyer il va falloir récuperer les données retourner par le service Web, les données sont géneralement envoyer sous un `format JSON`, il s'agit d'un format textuel dont la syntaxe est similaire à celle des objets Javascript, l'avantage est que ce format sera directement lus et transformer en objet Javascript par votre navigateur.

#### Qu'est-ce que Fetch

`Fetch` est un ensemble d'objets et de fonctions mis à disposition par le langage JavaScript, afin d'exécuter des requêtes HTTP de manière asynchrone. `Fetch` va nous renvoyer une Promise.
L’API Fetch  va nous permettre d'exécuter des requêtes HTTP sans avoir besoin de recharger la page du navigateur. Cela permet d':
...
`fetch` s'utilise comme suit: `fetch("http://url-service-web.com/api/users");`
...
L'utilisation de `fetch` pour envoyer ou recuperer des données via un service Web nous renvoie une `Promise`.

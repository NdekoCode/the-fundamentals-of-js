    ...
    for (let i = 0; i < n - 2; i++) {
        for (let j = i + 1; j < n - 1; j++) {
            for (let k = j + 1; k < n; k++) {
                ...
                if (
                    (JSON.stringify(newArray[i]) === JSON.stringify(newArray[j]))
                ) {
                    continue;

                }
                if (
                    (JSON.stringify(newArray[i] === JSON.stringify(newArray[k])))) {
                    continue;

                }
                if (
                    (JSON.stringify(newArray[j] === JSON.stringify(newArray[k])))) {
                    continue;
